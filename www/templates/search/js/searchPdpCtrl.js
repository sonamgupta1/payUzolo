appControllers.controller('searchPdpCtrl', function ($scope,productService,bookingService, $ionicHistory,$sce,$filter,
    ...
    $ionicModal.fromTemplateUrl('templates/search/html/addon_s.html', {
        scope: $scope,
    ...
    $scope.back_to_search_package = function() {
        $state.go('app.search_info', {'search_text': $stateParams.search_text});
    };
    
    $ionicModal.fromTemplateUrl('templates/search/html/review.html', {
        scope: $scope,
        animation: 'slide-in-up'
    }).then(function (modal) {
        $scope.modal = modal;
    ...
    $scope.shareProduct = function ($event,name,image,slug_url) {
        $mdBottomSheet.show({
            templateUrl: 'bottom-shared.html',
            controller: 'sharedSearchCtrl',
            targetEvent: $event,
            locals: {
                slug_url: slug_url,
                image:image,
    ...
    $scope.full_desc = function(id){
        $state.go('app.full_description_search',{'search_text': $stateParams.search_text,'cat_id': $stateParams.cat_id,product_id:id});
    };
